...
using Plots, LaTeXStrings, PlutoUI, Images
...
>f(x) = b^{x}
>```
>where ``b`` is a positive number with ``b \neq 1``
"""
...
md"""
#### Behaviour of Exponential Function ``f(x)=b^{x}``
>1. ``D(f) = (-\infty,\infty)`` 
>2. ``R(f) = \left[0,\infty\right)`` 
>3. ``f`` is an increasing function
>4. ``b^{x}`` is very large for large values of ``x``
>5. ``b^{x} \approx``  ``0`` if ``x`` is ``-ve`` and ``|x|`` is large
"""
...
md"""
#### Logarithm of 1 and the Logarithm of the base
>```math
>\log_{b}1 = 0
>```
>```math
...
md"""
#### Power Rule of Logaritham
>```math
>\log_{b}\;(y^{t}) = t\log_{b}y
>```
"""

# ╔═╡ 522111e9-5ab8-4461-b00e-33c58cbfa54f
md"""
#### Radioactive Decay and Half-Life 

Assume there are 100 atoms in a molecule and every 1 minute this will reduced to half. Let us simulate this
"""

# ╔═╡ 5387ee2b-a48d-4e95-9505-f81dcf55aa36
begin
	x₀ = 100
	global xₜ = x₀
	global res = Array{Float64}(undef,20)
	global i = 1
	while xₜ > 0.001
		global res[i] = xₜ
		global xₜ = xₜ/2
	...
	plot(0:19,res, title="Half-life-Simulation", label="y",xticks=0:19, yticks=0:10:100)
...
# ╔═╡ f97d8c82-ac9e-4213-aa0c-faa80bb01a4a
md"""
For a radon atom having a half life of ``92`` hours, we have below formula to find the percentage of remaining atoms after ``t`` hours
```math
\frac{1}{2^{\frac{t}{92}}}
```
"""

# ╔═╡ 63782045-aa53-45c7-b875-ca20531702db
md"""
>If a radioactive isotope has a half-life ``h``, then the function modeling the number of atoms in a sample of this isotope is 
>```math
>a(t) = a_{0} \cdot 2^{-t/h}
>```
...
# ╔═╡ a3fb704d-3b28-475d-ae06-ec1d31d7246d
md"""
##### Quizz 1:
Suppose a cat skeleton found in an old well has a ratio of carbon-14 to carbon-12
that is 61% of the corresponding ratio for living organisms. Approximately how
long ago did the cat die? Hint: Half life of carbon-14 is 5730 years
"""

# ╔═╡ 55d47d4e-239d-49ba-93e0-d0a4d43b399c
md"""
#### Change of Base for Logarithms
>if ``a,b`` and ``y`` are positive numbers, with ``a\neq1`` and ``b\neq1`` then 
>```math
>\log_{b}y = \frac{\log_{a}y}{log_{a}b}
>```
"""

# ╔═╡ a2901356-4bf6-46df-9376-c0b1366bed7d
md"""
#### Logarithm Porperties
>```math
...
>```
>```math
>\log_{b}\frac{1}{y} = -log_{b}y
>```
"""

# ╔═╡ 30af2caf-1dce-44b6-9875-7723f5528245
md"""
### Exponential Growth
**A doubling fable**:
A mathematician in ancient India invented the game of chess. Filled with gratitude for the remarkable entertainment of this game, the king offered the mathematician anything he wanted. The king expected the mathematician to ask for rare jewels or a majestic palace. But the mathematician asked only that he be given one grain of rice for the first square on a chessboard, plus two grains of rice for the next square, plus four grains for the next square, and so on, doubling the amount for each square, until the 64th square on an 8-by-8 chessboard had been reached. The king was pleasantly surprised that the mathematician had asked for such a modest reward beacuse the royal granary contain 200,000 bags. Is this reward modest ?
Hint: 1 grain = 0.020gm
"""

# ╔═╡ dac17929-6355-4d85-b07b-40b3ba8d8aad
md"""
#### Exponential Growth
>```math 
>f(x) = cb^{kx}
>```
> where ``c`` and ``k`` are positive numbers and ``b>1``
"""

# ╔═╡ 869c9fcf-ef62-4738-8ea1-f77a85dcd626
md"""
#### Logarithm of a function with exponential growth
>```math
>\log(y) = \log(cb^{kx})
>```
>```math
>\log(y) =  (k\log b)x + \log c
>```
graph of ``\log(f(x))`` is a straight line with positive slope
"""

# ╔═╡ fd855eb9-c3de-446f-8728-d48569c5ccf7
md"""
##### Quizz-2:Population Growth
...
# ╔═╡ 09780784-fbca-43df-86b6-c3a327bfbcc0
begin
	a₀ = 700
	t₀ = 13
	tₙ = 21
	aₙ = Array{Float64}(undef,tₙ - t₀+1)
	for t = 0:tₙ-t₀
		 aₙ[t+1] = a₀*2^(t/3)
	end
	plot(aₙ, title="Bacteria Growth",label=L"700 \times 2^{t/3}",xlabel="Time",ylabel="Population")
end

# ╔═╡ 5d2dddef-229c-4ab7-8050-00e8bc1a599f
md"""
#### Compound Interest
"""

# ╔═╡ fec689b2-e231-451c-b4dc-794dd2f9799c
 load("fun.png")
...
begin
	...
	yrs = 1:5
	sₜ = @.s₀*(1+(rt/100)*yrs)
	plot(sₜ, title="Simple Interest",label=L"P(1+rt)", xlabel="years",ylabel="amount")
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "ca1f4523bbfefafe47b31dde2dbb02ecbe258983"
